...
pca.grid = function(pca.obj=pca.res, col='darkblue', diff.samples = NULL,
                    default.label = "Matching", diff.label = "RNA-seq only", point.label=F){
  ...
  gg.pca = function(pca=pca.res, PCs = c(1,2), label = point.label ){
    ...
    if(is.null(diff.samples)){
      ...
      if(label){
        p = p + geom_point(aes(x=mat[,PCs[1]], y=mat[,PCs[2]],
                               label=rownames(mat)), col =col)+ geom_text(aes(label=rownames(mat)))
      } else {
