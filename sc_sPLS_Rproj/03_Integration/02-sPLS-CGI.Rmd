...
```{r}
## make bed file for CGI
library(annotatr)
CGI_annotation <- build_annotations('hg38', annotations = 'hg38_cpg_islands')
CGI_bed <- CGI_annotation %>% as.data.table() %>% .[,c(1:3,5:6)] %>%
  setnames(c("chr","start","end","strand","id")) %>% .[,anno:='CGI'] %>% 
  .[chr %in% paste0('chr',c(1:22,'M'))] %>% .[,strand:='+'] %>% .[,chr:=chr_conv(chr, style = 'ensembl')] %>% .[,id:=gsub(':','_',id)]
write.table(CGI_bed, sep="\t",file = '../../data/scBSseq/filt/CGI.bed', row.names = FALSE, col.names = FALSE)
out <- fread('../../data/scBSseq/filt/CGI.bed' ,sep="\t", header=F, verbose=F)
out
```

```{r}
CGI <- readRDS('../../data/scBSseq/met/CGI.rds')
CGIstats <- readRDS('../../data/scBSseq/met/CGIwithStats.rds')
...
GOget.res <- GOget(cgi_spls, dat="Y", comp=1,  ID="ensembl", mapping="org.Hs.eg.db", feas_genes = NULL )
goEnrich.res <- goEnrich(GOget.res)
GOplot.res <- GOplot(goEnrich.res, trms = list(leuk='leukemia'), maxEnrich = 5, col = color.mixo(c(1,8)))
...
```{r}
pdf('img/blockspls-cgi-intergenic-plotVar.pdf')
plotVar(blockspls, pch=c(17,16,15), legend = TRUE)
dev.off()
```
...
```{r}
pdf('img/blockspls-cgi-intergenic-plotVar.pdf', width = 11, height = 8)
